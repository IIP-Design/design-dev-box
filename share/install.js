...
var schema = {
	properties: {
		ip: {
			description: 'The default ip is 192.168.33.43.  Hit enter to accept default or type new ip'
		}
	}
}
...
function promptForInstructions( callback ) {
	...
	msg('');
	prompt.start();
	prompt.get(['This install will create a design.dev domain on your local machine.\nIf a design.dev domain already exists, then this will create a conflict\nRemove or rename it before continuing.\nHit enter when you are ready to continue....'], function() {
		callback();
	})
}
...
function cloneSite ( callback ) {
	msg('Cloning the Share repository');
	git.clone( 'https://github.com/USStateDept/ShareAmerica', 'www', function( err ) {
		callback();
	});
}
...
function moveConfig ( callback )  {
	msg('Moving wp_config to Share dir');
